...
if (taxType === "cgst_sgst") {
  ...
  igstAmount = (totalAmount * 18) / 100;
  grandTotal = totalAmount + igstAmount;
} 
else {
  grandTotal = totalAmount; // No tax
}
...
setUpdatedDetails({
  ...
  const handleSubmit = () => {
  const updatedData = { ...updatedDetails, taxType };
...
<select
 ...
 onChange={(e) => {
  ...
  const total = Number(updatedDetails.totalAmount || 0);

  let cgst = 0, sgst = 0, igst = 0, grand = total;

  if (selected === "cgst_sgst") {
    cgst = (total * 9) / 100;
    sgst = (total * 9) / 100;
    grand = total + cgst + sgst;
  } 
  else if (selected === "igst") {
    igst = (total * 18) / 100;
    grand = total + igst;
  } 
  else {
...
{taxType === "cgst_sgst" && (
  <>
    <label>CGST (9%):</label>
    <input value={updatedDetails.cgstAmount || 0} readOnly />

    <label>SGST (9%):</label>
    <input value={updatedDetails.sgstAmount || 0} readOnly />
  </>
)}
...
{taxType === "igst" && (
  <>
    <label>IGST (18%):</label>
    <input value={updatedDetails.igstAmount || 0} readOnly />
  </>
)}
